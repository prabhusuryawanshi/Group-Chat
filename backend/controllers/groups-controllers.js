...
const fetchGroups = async (req, res, next) => {
  ...
  try {
    ...
    return next(new Error('Could not fetch groups: ' + error));
  }
...
const fetchGroupData = async (req, res, next) => {
  ...
  try {
    ...
    return next(new Error('Could not fetch groups by id: ' + error));
  }
...
const createGroup = async (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    return next(new Error('Invalid entries: ' + error));
  }
  ...
  try {
    ...
    return next(new Error('Could not save group to DB: ' + error));
  }
