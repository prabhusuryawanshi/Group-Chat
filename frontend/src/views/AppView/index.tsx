...
import ENV from '../../utility/environemt';
...
const AppView: React.FC = () => {
  ...
  const createGroup = async (title: string, description: string) => {
    ...
    let verifiedToken;
    try {
      verifiedToken = await axios.post(`${ENV.REACT_APP_SERVER_URL}/users/verify`, {
        id,
        token
      });
    } catch (error) {
    ...
    let response;
    try {
      response = await axios.post(`${ENV.REACT_APP_SERVER_URL}/groups`, {
        title,
  ...
  const editProfileRequest = async (username: string, image: string) => {
    ...
    let response;
    try {
      response = await axios.put(`${ENV.REACT_APP_SERVER_URL}/users/edit`, {
        id,
        username,
        image
      });
  ...
  const createMessage = async (text: string, date: string) => {
    ...
    let response;
    try {
      response = await axios.post(`${ENV.REACT_APP_SERVER_URL}/messages`, {
  ...
  const fetchGroups = async () => {
    let response;
    try {
      response = await axios.get(`${ENV.REACT_APP_SERVER_URL}/groups`);
    } catch (error) {
  ...
  const fetchMessages = async (gid = currentGroup?._id) => {
    ...
    let response;
    try {
      response = await axios.get(`${ENV.REACT_APP_SERVER_URL}/groups/${gid}`);
    } catch (error) {
  ...
  const reportBug = async (title: string, description: string) => {
    ...
    let response;
    try {
      response = await axios.post(`${ENV.REACT_APP_SERVER_URL}/bugs`, {
        id,
        title,
        description: description ? description : 'No description.'
      });
